...
项目已整理好 DBeaver 脚本，目录 **`DBeaver/Scripts/`**，索引见 [DBeaver/Scripts/README.md](../DBeaver/Scripts/README.md)。

| 前缀 | 含义 |
|------|------|
| `查询-` | 只读，可直接执行 |
| `写入-` | ⚠️ **会改库**，文件内分「预览」与「写入」两段，先预览、确认后再执行写入段 |

在 DBeaver：**SQL 编辑器 → 打开文件**，选中一段后 **Ctrl+Enter** 只执行选中部分。
...
或打开 `DBeaver/Scripts/查询-重复号码.sql`。
...
## 10. DBeaver 与 LibreOffice Base 怎么选
